...
It is very similar to previous [dataset-join-02](dataset-join-02.md). However, this is independent from `spark-shell` or `spark-submit`. Both save back results to a Cassandra's table. Furthermore, dataset-join-03 writes to disk on a JSON format output file.
